...
export async function uploadVideo(req, res, next) {
  try {
    ...
    if (existingVideo) {
      ...
      if (uploaderId) {
        await linkVideoToUploader(hash, uploaderId, { isDuplicate: true, uploadedAt: new Date() });
      }
    ...
    const videoData = {
      ...
      validation: metadata.validation,
      blockchainTxn: blockchainResult.txHash || null,
    ...
    if (eventId && eventType) {
      await createEvent({
        id: eventId,
        type: eventType,
        tags: eventTags ? eventTags.split(',') : [],
        description: description
      });
      await linkVideoToEvent(hash, eventId, { 
        eventType,
        recordedAt: claimedTimestamp || new Date()
      });
    ...
    if (location || gpsCoordinates) {
      const locationId = await createLocation({
        ...
        timestamp: claimedTimestamp || new Date()
      });
